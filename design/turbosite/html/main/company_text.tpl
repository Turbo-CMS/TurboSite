<div class="drag-block container COMPANY_TEXT" data-class="company_text_drag" data-order="{$theme_settings->block_5}">
					...
					<div class="flexbox flexbox--direction-row company-front-wrapper outer-rounded-x relative company-image-type {if $theme_settings->company_text_bordered}bordered{/if}">
							...
							<div class="sticky-block company-item__info">
								{if $theme_settings->company_text_image}
									<div class="company-item__picture-wrapper">
										<div class="lazyload company-item__picture rounded-x" style="background-image:url(data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==)" data-bg="design/{$settings->theme|escape}/images/abaut_site.jpg">
										</div>
									</div>
								{else}
									<div class="company-item__text index-block__preview">
						...
						{if $theme_settings->company_text_button}
							<div class="index-block__btn">
								<a href="{$lang_link}{$theme_settings->company_text_url}" class="btn btn-default btn-elg">{$lang->about_company|escape}</a>
							</div>
						{/if}
					</div>
				</div>
