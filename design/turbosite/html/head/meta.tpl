...
{if $project}
	...
	<meta property="og:image" content="{if $project->image}{$project->image->filename|resize:330:300}{/if}">
	<link rel="image_src" href="{if $project->image}{$project->image->filename|resize:330:300}{/if}">
	{* Twitter *}
	...
	<meta name="twitter:image" content="{if $project->image}{$project->image->filename|resize:330:300}{/if}">
...
{foreach $languages as $lang}
	{if $lang->enabled}
		<link rel="alternate" hreflang="{if $lang@first}x-default{else}{$lang->label|escape}{/if}" href="{$config->root_url}{if !$lang@first}/{/if}{$lang->url|escape}">
	{/if}
{/foreach}