let serviceUrl = 'ajax/search_pages.php';
let type;
	...
	$input.each(function () {
		...
		$this.devbridgeAutocomplete({
			...
			formatResult: function (suggestion, currentValue) {
				...
				var urlPrefix = type === 'projects' ? 'project/' : type === 'blog' ? 'blog/' : type === 'article' ? 'article/' : '';
...
$(document).on('click', '.dropdown-select.searchtype .dropdown-menu-item:not(.dropdown-menu-item--current)', function () {
	...
	switch (type) {
		case 'all':
			...
			break;
		case 'projects':
			name = Turbo.message('SEARCH_IN_PROJECTS');
			break;
		case 'blog':
	...
	$('.dropdown-select.searchtype').each(function () {
		...
		switch (type) {
			case 'all':
				...
				break;
			case 'projects':
				url = turboOptions.PAGES.PROJECTS_PAGE_URL;
				break;
			case 'blog':
