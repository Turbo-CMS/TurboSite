...
{/if}
{if $cats}
	<h2 class="my-4">{$lang->index_projects}</h2>
	{function name=cat_prod}
		{if $prod}
			<ul>
				{foreach $prod as $p}
					{if $p->visible}
						<li><a class="text-decoration-none" href="{$lang_link}projects/{$p->url}">{$p->name}</a></li>
					{/if}
				{/foreach}
			</ul>
		{/if}
		...
		{if $projects}
			<ul>
				{foreach $projects as $p}
					{if $p->visible}
						<li><a class="text-decoration-none" href="{$lang_link}project/{$p->url}">{$p->name}</a></li>
					{/if}
				{/foreach}
			</ul>
		{/if}
		...
		{if $projects_cats}
			<ul>
				{foreach $projects_cats as $c}
					{if $c->visible}
						<li>
							<a class="text-decoration-none" href="{$lang_link}projects/{$c->url}"><b>{$c->name}</b></a>
