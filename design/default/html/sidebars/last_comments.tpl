		...
		<div class="card-body">
			...
			{if $last_comments}
				{foreach $last_comments as $comment name=com}
					...
					<div class="mb-1"><small class="text-muted">{$comment->date|date} {$lang->at} {$comment->date|time}</small></div>
					<div class="mb-1">{$comment->text|strip_tags|truncate:150}</div>
					<div class="mb-1">
						{if $comment->type == project}
							<a href="{$lang_link}project/{$comment->project->url}#comment_{$comment->id}" class="text-decoration-none">
								<small>{$comment->project->name}</small>
							</a>
						{/if}
						...
						{if $comment->type == review}
							<a href="{$lang_link}reviews#comment_{$comment->id}" class="text-decoration-none">
								<small>{$lang->reviews_global}</small>
							</a>
						{/if}
					</div>
