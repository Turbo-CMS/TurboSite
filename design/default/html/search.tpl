...
<div class="search-result-box card-box mb-5">
	...
	<div class="row">
		<div class="col-md-12">
			{if $keyword}
				{if $posts || $pages_search || $articles || $projects}
					...
					{foreach $projects as $project}
						<div class="search-item mt-3">
							<h4 class="mb-1"><a data-project="{$project->id}" href="{$lang_link}project/{$project->url}">{$project->name|escape}</a></h4>
							<div class="font-13 text-success mb-3">{$config->root_url}/project/{$project->url}</div>
							<small class="text-muted">{$project->date|date}</small>
							<p class="mb-0 text-muted">{$project->annotation|strip_tags|truncate:250}</p>
						</div>
					{/foreach}
