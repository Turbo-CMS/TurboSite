	...
	<div class="container-xxl">
		...
		<div class="collapse navbar-collapse" id="navbarMenu">
					...
					{if $p->menu_id == $theme_settings->menu_header_id}
						{if $p->visible}
							<li itemprop="name" class="nav-item dropdown {if $page && $page->id == $p->id}active{/if}">
								<a itemprop="url" class="nav-link {if $p->subpages}dropdown-toggle{/if}" href="{$lang_link}{$p->url}" id="dropdown{$p->id}" data-page="{$p->id}" {if $p->subpages}data-bs-toggle="dropdown" aria-expanded="false" {/if} aria-haspopup="true">{$p->header|escape}</a>
								{if $p->subpages}
									<div class="dropdown-menu" aria-labelledby="dropdown{$p->id}">
										{foreach $p->subpages as $p2}
											<a itemprop="url" data-page="{$p2->id}" class="dropdown-item" href="{$lang_link}{$p2->url}">{$p2->header|escape}</a>
										{/foreach}
									</div>
								{/if}
							</li>
			...
			<ul class="nav navbar-nav navbar-right">
				...
				{* Login *}
				{if $user}
					<li class="nav-item"><a class="nav-link text-decoration-none" href="{$lang_link}user"><i class="fal fa-user text-primary me-2"></i>{$user->name|escape}</a></li>
...
<div class="container-xxl pt-4 mt-5">
	<div class="row justify-content-between mt-2">
		...
		<div class="col-lg-6">
			<div class="text-end">
				<div class="mb-1">{$theme_settings->phone_1|escape}</div>
				<div class="mb-3">{$lang->contact_details|escape}</div>
				<div class="callback float-end">
					<a class="btn btn-primary btn-sm" href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#callbackModal" role="button">
