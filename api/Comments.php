...
class Comments extends Turbo
{
	...
	public function get_comments($filter = array())
	{
		...
		$query = $this->db->placehold("SELECT c.id, c.object_id, c.ip, c.name, c.text, c.type, c.date, c.approved, c.parent_id, c.admin, c.rate, c.rating
										$blog_fields $articles_fields $projects_fields FROM __comments c $blog_join $articles_join $projects_join WHERE 1 $object_id_filter $type_filter $keyword_filter $approved_filter ORDER BY id $sort $sql_limit");
	...
	public function add_comment($comment)
	{
		...
		if ($comment['approved'] == 1 && $comment['object_id']) {
			if ($comment['type'] == 'blog') {
				...
				$this->db->query('update __projects set last_modified=now() where id=?', intval($comment['object_id']));
	...
	public function delete_comment($id)
	{
		if (!empty($id)) {
			...
			if ($c->approved == 1) {
				...
				} elseif ($c->type == 'project') {
					$this->db->query('update __projects set last_modified=now() where id=?', intval($c->object_id));
