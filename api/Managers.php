...
class Managers extends Turbo
{
	...
	{
		// For compatibility with CGI mode
	...
	{
		// If not requested by login, return the current manager or false
		if (empty($login))
			if (!empty($_SERVER['PHP_AUTH_USER']))
				$login = $_SERVER['PHP_AUTH_USER'];
			else {
				// Test manager if authorization is disabled
				$m = new stdClass();
				$m->login = 'manager';
