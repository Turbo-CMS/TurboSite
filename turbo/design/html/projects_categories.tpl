...
<div class="d-md-flex mb-3">
	...
	<div class="d-grid gap-2 d-sm-block mt-2 mt-md-0">
		<a class="btn btn-primary" href="{url module=ProjectsCategoryAdmin return=$smarty.server.REQUEST_URI}"><i data-feather="plus"></i> {$btr->categories_add|escape}</a>
	</div>
</div>
										...
										<div class="js-row {if $level == 1}turbo-list-body-item{/if} js-sort-item body-narrow row-narrow">
											<div class="turbo-list-row {if $level > 1}js-sort-item{/if} narrow">
												...
												<div class="turbo-list-boding turbo-list-categories-name">
													<a class="fw-bold text-body text-decoration-none" href="{url module=ProjectsCategoryAdmin id=$category->id return=$smarty.server.REQUEST_URI}">
														{$category->name|escape}
													</a>
												</div>
												<div class="turbo-list-boding turbo-list-status">
													<div class="form-check form-switch">
														<input class="form-check-input js-ajax-action {if $category->visible}js-active-class{/if}" id="id_{$category->id}" data-module="projects_category" data-action="visible" data-id="{$category->id}" name="visible" value="1" type="checkbox" {if $category->visible}checked="" {/if}>
														<label class="form-check-label" for="id_{$category->id}"></label>
													</div>
												</div>
												<div class="turbo-list-setting">
													<a href="../projects/{$category->url}" target="_blank" class="setting-icon setting-icon-open" data-bs-toggle="tooltip" data-bs-placement="top" title="{$btr->global_view|escape}">
