					...
					<div class="row">
						<div class="col-lg-3 col-md-3 col-sm-12 mb-3">
							<select class="selectpicker" onchange="location = this.value;">
								...
								<option value="{url keyword=null type=project}" {if $type == 'project'}selected{/if}>{$btr->comments_to_projects|escape}</option>
								...
								<option value="{url keyword=null type=review}" {if $type == 'review'}selected{/if}>{$btr->global_review|escape}</option>
							</select>
						</div>
					</div>
				</div>
							...
							<div class="turbo-list-body">
								{function name=comments_tree level=0}
									{foreach $comments as $comment}
										<div class="js-row turbo-list-body-item {if !$comment->approved}unapproved{/if} {if $level > 0}admin-note2{/if}">
											<div class="turbo-list-row">
												...
												<div class="turbo-list-boding turbo-list-comments-name {if $level > 0}admin-note{/if}">
													...
													<div class="mb-0">
														...
														<span class="text-secondary">{$comment->text|escape|nl2br}</span>
													</div>
													<div class="mb-0">
														...
														{if $comment->type == 'project'}
															{$btr->comments_project|escape}<a target="_blank" href="{$config->root_url}/projects/{$comment->project->url}#comment_{$comment->id}" class="fw-bold text-body text-decoration-none">{$comment->project->name}</a>
														{elseif $comment->type == "blog"}
															{$btr->comments_blog|escape}<a target="_blank" href="{$config->root_url}/blog/{$comment->post->url|escape}#comment_{$comment->id}" class="fw-bold text-body text-decoration-none">{$comment->post->name|escape}</a>
														{elseif $comment->type == 'article'}
															{$btr->comments_article|escape}<a target="_blank" href="{$config->root_url}/article/{$comment->article->url}#comment_{$comment->id}" class="fw-bold text-body text-decoration-none">{$comment->article->name}</a>
														{elseif $comment->type == 'review'}
															<a target="_blank" href="{$config->root_url}/reviews/#comment_{$comment->id}" class="fw-bold text-body text-decoration-none">{$btr->global_review|escape}</a>
														{/if}
													</div>
													<a href="index.php?module=CommentsAdmin&item_id={$comment->object_id}&type={$comment->type}" class="comment-object fw-bold text-body text-decoration-none">{$btr->all_comments_to_object|escape} <i class="align-middle" data-feather="arrow-right"></i></a>
												...
												<div class="turbo-list-boding turbo-list-comments-btn">
													...
													{/if}
													<div class="answer-wrap js-answer_btn" {if $level > 0 || !$comment->approved}style="display: none;"{/if}>
														<button type="button" data-parent-id="{$comment->id}" data-user-name="{$comment->name|escape}" data-bs-toggle="modal" data-bs-target="#answer-popup" class="btn btn-outline-primary js-answer">
															{$btr->global_answer|escape}
														</button>
													</div>
