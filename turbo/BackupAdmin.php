		...
		if ($this->request->method('post')) {
			switch ($this->request->post('action')) {
				case 'create': {
						...
						$v_list = $zip->create(array('files', $dir . 'turbo.sql'), PCLZIP_OPT_REMOVE_PATH, $dir, PCLZIP_CB_PRE_ADD, "myCallBack");
						if ($v_list == 0) {
...
{
	// Check the success of unpacking
...
function myCallBack($p_event, &$p_header)
{
	...
	if (preg_match('/^files\/projects\/.+/i', $fname))
		return 0;
	elseif (preg_match('/^files\/articles\/preview\/.+/i', $fname))
		return 0;
	elseif (preg_match('/^files\/categories\/preview\/.+/i', $fname))
		return 0;
	elseif (preg_match('/^files\/posts\/preview\/.+/i', $fname))
		return 0;
	elseif (preg_match('/^files\/slides\/preview\/.+/i', $fname))
		return 0;
	return 1;
}
